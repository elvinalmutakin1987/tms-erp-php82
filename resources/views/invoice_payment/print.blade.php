...
<table class="table-p2h">
    <thead>
        <tr>
            <th colspan="8" class="doc-header-wrapper">
                <table class="doc-header-table">
                    <tr>
                        ...
                        <td class="title-cell">
                            <div class="doc-title">Invoice Payment</div>
                        </td>

                        <td class="meta-cell">
                            <div class="docno-label">Document No.</div>
                            <div class="docno">{{ $invoice_payment->payment_no }}</div>
                        </td>
                    </tr>
                </table>
            </th>
                ...
                <table style="width: 100%; border-collapse: separate; border-spacing: 0;">
                    <tr>
                        ...
                        <td style="border: none;">
                            {{ $invoice_payment->invoice->client_vendor?->name ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <td style="border: none; width: 25%">
                    ...
                    <tr>
                        ...
                        <td style="border: none;">
                            {{ $invoice_payment->bank_receipt }} -
                            {{ $invoice_payment->bank_account_receipt }}
                        </td>
                    </tr>
                    <tr>
                        <td style="border: none; width: 25%">
                    ...
                    <tr>
                        ...
                        <td style="border: none;">
                            {{ $invoice_payment->total ? Number::format($invoice_payment->total, precision: 0) : '' }}
                        </td>
                    </tr>
                    <tr>
                        <td style="border: none; width: 25%">
        ...
        <tr>
            <td colspan="8" class="p-1">
                <table style="width: 100%; border-collapse: separate; border-spacing: 0;">
                    ...
                    <tr>
                        <td style="border: none; text-align: center">
                            @if ($invoice_payment->user->sign_path)
                                <img src="{{ public_path('storage/' . $invoice_payment->user->sign_path) }}"
                                    alt="Logo" style="max-width:100px;height:auto;margin:0 auto;">
                            @endif
                            <br>
                            {{ $invoice_payment->user->name }}
                        </td>
                        @if ($approval_step)
                            @foreach ($approval_step as $d)
                                <td style="border: none; text-align: center">
                                    @php
                                        $approval_status = $approval_status
                                            ...
                                            ->where('approvable_id', $invoice_payment->id)
                                            ->where('step', $d->order)
                                            ->first();
                                    @endphp
