...
<script>
    ...
    $('.saveButton').on('click', function() {
        ...
        if (orderId) {
            url = '{{ route('
            purchaseorder.update ', ': _id ') }}'.replace(':_id', orderId);
    ...
    $('.monitoringButton').on('click', function() {
        ...
        let url = '{{ route('
        purchaseorder.monitoring ', ': _id ') }}'.replace(':_id', orderId);
        let type = 'POST';
    ...
    $('#formModal').on('show.bs.modal', function() {
        ...
        setTimeout(function() {
            ...
            const url = isEdit ?
                '{{ route('
            purchaseorder.get_table_edit ', ': _id ') }}'.replace(':_id',
                    orderId):
                '{{ route('
            purchaseorder.get_table_add ') }}';
            $.ajax({
                url: url,
                data: {
    ...
    function delete_file(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url = '{{ route('
                purchaseorder.destroy_file ', ': _id ') }}';
                url = url.replace(':_id', id);
                $.ajax({
                    url: url,
                    type: 'DELETE',
    ...
    function loadClientVendorTaxable(vendorId) {
        ...
        let url = '{{ route('
        purchaseorder.get_client_vendor_by_id ', ': _id ') }}';
    ...
    function initPurchaseRequisitionSelect2() {
        ...
        $purchase_requisition.select2({
            ...
            ajax: {
                url: '{{ route('
                purchaseorder.get_purchase_requisition ') }}',
                dataType: 'json',
    ...
    function initClientVendorSelect2(triggerTaxable = true) {
        ...
        $vendor.select2({
            ...
            ajax: {
                url: '{{ route('
                purchaseorder.get_client_vendor ') }}',
                dataType: 'json',
                data: function(params) {
                    return {
                        term: params.term || '',
