...
@section('content')
                            ...
                            <div class="row align-items-center">
                                <div class="col">
                                    <a href="javascript:;" id="openModalButton" class="btn btn-primary mb-3 mb-lg-0"
                                        ...
                                        data-title="Add Progress Claim"><i class='bx bxs-plus-square'></i>New</a>
                                </div>
                                <div class="col">
    ...
    <!--end page wrapper -->
    @include('progress_claim.modal', $contract)

    @include('progress_claim.modal-edit', $contract)

    @include('progress_claim.modal-detail')

    @include('progress_claim.modal-update')
@endsection
    ...
    <script>
        ...
        async function loadProformaInvoiceTable() {
            ...
            loadTableTimer = setTimeout(function() {
                ...
                const url = isEdit ?
                    '{{ route('progressclaim.get_table_edit', ':_id') }}'.replace(':_id',
                        proformaInvoiceId) :
                    '{{ route('progressclaim.get_table_add') }}';
        ...
        function checkProformaInvoice() {
            ...
            var month = $('#month').val();
            return $.ajax({
                    url: '{{ route('progressclaim.check_proforma_invoice') }}',
                    type: 'GET',
                    data: {
                        contract_id: contractId,
                        year: year,
        ...
        $(document).off('click.editButton').on('click.editButton', '.editButton', function() {
            ...
            $('#modal-edit-header').text('Edit Progress Claim');
            $('#id').val(proformaInvoiceId);

            let url = '{{ route('progressclaim.show', ':_id') }}';
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('#modal-edit-header').html(
                        'Edit Progress Claim -&nbsp;<b>' + response.proforma_no + '</b>'
                    );
        ...
        $(document).off('click.detailButton').on('click.detailButton', '.detailButton', function() {
            $('#modal-detail-header').text('Detail Progress Claim');

            let url = '{{ route('progressclaim.get_detail', ':_id') }}';
        ...
        $('.saveButton').off('click.saveProforma').on('click.saveProforma', function() {
            ...
            let url = '{{ route('progressclaim.store') }}';

            formData.append('status', statusValue);

            if (proformaInvoiceId !== '') {
                url = '{{ route('progressclaim.update', ':_id') }}'
        ...
        $('.saveUpdateButton').off('click.updateProforma').on('click.updateProforma', function() {
            ...
            if (proformaInvoiceId !== '') {
                url = '{{ route('progressclaim.update_progress', ':_id') }}'
                    .replace(':_id', proformaInvoiceId);
        ...
        function delete_(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let url = '{{ route('progressclaim.destroy', ':_id') }}';
