    ...
    */
    ...
    foreach ($dateGroups as $group) {
        ...
        for ($column = 0; $column < 10; $column++) {
            $day = $group->get($column);

            if ($day) {
                $dateKey = Carbon::create($year, $month, $day)->format('Y-m-d');
                $exactHours = $hoursByDate[$dateKey] ?? 0;
                $hoursRounded = (int) round($exactHours, 0, PHP_ROUND_HALF_UP);
    ...
    <tr>
        <td style="width:50%;">
            <table class="doc-header-detail">
                <tr>
                    <td style="width:18%;">Kode Unit</td>
                ...
                <tr>
                    ...
                    <td style="text-align:center;">:</td>
                    <td>
                        {{ $unit?->unit_brand?->name ?? '' }}
                        {{ $unit?->unit_model?->desc ?? '' }}
                    </td>
                </tr>
                <tr>
                    <td>Periode</td>
    ...
    @foreach ($operationGroups as $group)
        ...
        <tr>
            @foreach ($group['hours'] as $hours)
                ...
                @else
                    <td class="hour-cell empty-cell">&nbsp;</td>
                @endif
            @endforeach
...
<table class="signature-table avoid-break">
    <tr>
        <td colspan="3" class="signature-date">
            Sangatta,
